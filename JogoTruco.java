package cartas.truco;// cartas.truco.JogoTruco.java
...
public class JogoTruco extends JFrame {
    ...
    private void inicializarJogadoresEDuplas() {
        ...
        duplas = new DuplaTruco[]{new DuplaTruco(0), new DuplaTruco(1)};
    }
    ...
    private void criarPainelPontos() {
        ...
        lblPontuacao = new JLabel("cartas.truco.DuplaTruco 0: 0   |   cartas.truco.DuplaTruco 1: 0", JLabel.CENTER);
    ...
    private void criarPainelJogo() {
        ...
        for (int i = 0; i < 4; i++) {
            labelsMesa[i] = new JLabel("cartas.truco.JogadorTruco " + i, JLabel.CENTER);
    ...
    private JPanel criarPainelJogador(int jogadorId) {
        ...
        painel.setBorder(BorderFactory.createTitledBorder("cartas.truco.JogadorTruco " + jogadorId));

        JPanel painelCartas = new JPanel(new FlowLayout());
        for (int i = 0; i < 3; i++) {
            JButton btn = new JButton("cartas.framework.Carta " + (i + 1));
    ...
    private void distribuirCartas() {
        ...
        baralho.embaralhar();
        for (JogadorTruco j : jogadores) j.getMao().getCartas().clear();
    ...
    private void atualizarInterface() {
        ...
        String estado = "Rodada " + (rodadaAtual + 1) + " — Vez do cartas.truco.JogadorTruco " + jogadorAtual;
        if (aguardandoRespostaTruco) {
            int duplaResponde = 1 - jogadores[jogadorQuePediuTruco].getDuplaId();
            estado += " — cartas.truco.DuplaTruco " + duplaResponde + " responde ao TRUCO!";
        }
        lblEstado.setText(estado);
        lblPontuacao.setText(String.format("cartas.truco.DuplaTruco 0: %d   |   cartas.truco.DuplaTruco 1: %d",
            ...
            if (placarParcial[0] == 2 || placarParcial[1] == 2 ||
                ...
                if (duplas[0].getPontuacao() >= Constants.PONTOS_VITORIA ||
                    ...
                    JOptionPane.showMessageDialog(this,
                            (duplas[0].getPontuacao() >= Constants.PONTOS_VITORIA ? "cartas.truco.DuplaTruco 0" : "cartas.truco.DuplaTruco 1") +
                                    " VENCEU O JOGO!");
                    System.exit(0);
                }

                JOptionPane.showMessageDialog(this, "Mão terminada! cartas.truco.DuplaTruco " + duplaMao + " ganhou.");
                reiniciarMao();
            } else {
                // ✅ LIMPAR A MESA AO FINAL DA VAZA
                for (int i = 0; i < 4; i++) {
                    labelsMesa[i].setText("cartas.truco.JogadorTruco " + i);
                    labelsMesa[i].setBackground(Color.WHITE);
                }
                rodadaAtual++;
    ...
    private void correrTruco(int jogadorId) {
        ...
        JOptionPane.showMessageDialog(this, "cartas.truco.DuplaTruco " + duplaPediu + " ganhou " + pontos + " ponto(s)!");
        reiniciarMao();
    }
